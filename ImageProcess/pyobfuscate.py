...
def check_result(func):
    if ENABLE_DEBUG:
        def wrap(*args, **kargs):
            ret=func(*args, **kargs)
            if ret:
                print >>sys.stderr,"\t\t -> obfuscate: True"
            else:
                print >>sys.stderr,"\t\t -> obfuscate: False"
            return ret
        return wrap
    else:
        return func
...
class CSTWalker:
    ...
    def should_obfuscate(self, id, symtabs):
        ...
        if ENABLE_DEBUG:
            tabtype__t = tab.get_type()
            print  >>sys.stderr,"--->", id
            print  >>sys.stderr,'\t tabtype:',tabtype__t
            print  >>sys.stderr,"\t pubapi:",self.pubapi
            print  >>sys.stderr,"\t isglobal:",s.is_global()
            print  >>sys.stderr,"\t islocal:",s.is_local()
            print  >>sys.stderr,"\t isimported:",s.is_imported()
            print  >>sys.stderr,"\t isparameter:",s.is_parameter()
            print  >>sys.stderr,"\t isinstance:",isinstance(tab, LambdaSymTable)
...
class TokenPrinter:
    ...
    def play(self, f):
        for tok in tokenize.generate_tokens(f.readline):
            ...
            if ENABLE_DEBUG:
                print >>sys.stderr, "\t --> lineno::", self.this_lineno, "srow::", t_srow_scol[0], "type::", t_type, "t_string::", t_string
    ...
    def line_append(self, s):
        if ENABLE_DEBUG:
            print >>sys.stderr, "\t\t ", self.pending, s
        if self.pending:
